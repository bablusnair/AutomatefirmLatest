...
#import "addnewstoreprinterclass.h"
#import "printercellclass.h"
#import "settingsViewController.h"

@implementation addnewstoreprinterclass
...
-(void)awakeFromNib
{
    ...
    //self.myoficeDictionary = [[NSUserDefaults standardUserDefaults]objectForKey:@"store_selectedprinter"];
    
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"printerstoredictionary"]==Nil) {
        
        NSMutableArray *printerarray = [[NSUserDefaults standardUserDefaults]objectForKey:@"store_selectedprinter"];
        ...
        if (printerarray.count>0) {
            ...
            for (int i=0; i<printerarray.count; i++) {
                ...
                [self.prnternamearray addObject:[dict objectForKey:@"printer"]];
...
-(IBAction)donebuttonaction:(id)sender
{
    ...
    if ((self.prnternamearray.count==self.emailarray.count)&&(self.prnternamearray.count==self.rateaarray.count)){
        ...
        [[NSUserDefaults standardUserDefaults] setObject:saveprinterdict forKey:@"printerstoredictionary"];
...
-(BOOL)textFieldShouldEndEditing:(UITextField *)textField
{
    ...
    if (textField==cell1.namefield) {
        if (cell1.namefield.text.length > 0) {
            NSLog(@"%d",myindex.row + 1);
            NSLog(@"%d",self.prnternamearray.count);
