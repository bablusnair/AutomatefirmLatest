...
@implementation advanceTileClass
...
- (void)touchDown:(id)sender {
   ...
   // [[NSUserDefaults standardUserDefaults] setInteger:selectedIndexValue forKey:@"tileId"];
        [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%d",selectedIndexValue] forKey:@"tile"];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"viewindividualfunction" object:Nil];
        ...
        if (allowsMultipleSelection) {
            ...
            advancesettingsviewclass *objj = (advancesettingsviewclass *)self.superview;
        ...
        UIAlertAction* ok = [UIAlertAction actionWithTitle:@"Discard Changes" style:UIAlertActionStyleDefault
                                                  ...
                                                  //     AppDelegate *myappde =(AppDelegate *)[[UIApplication sharedApplication]delegate];
                                                       NSString * stid=[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults]objectForKey:@"tile"]];
                                                       if ([myappde.advanceTileIdDict objectForKey:stid]==Nil) {
                                                             [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelfunction" object:Nil];
                                                       }
//                                                      else
//                                                   [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelfunction" object:Nil];
                                                   }];
...
-(void)deleteResponse:(id)response
{
    ...
    [myappde.advanceTileIdDict removeObjectForKey:str];
       
    NSMutableDictionary *dict=[[[NSMutableDictionary alloc] initWithDictionary:myappde.advanceTileIdDict] mutableCopy];
    NSMutableArray *keyaray=[[dict allKeys]mutableCopy];
    [myappde.advanceTileIdDict removeAllObjects];
    ...
    for (int i=0; i<AscendingArray.count; i++) {
        ...
        [myappde.advanceTileIdDict setObject:value forKey:[NSString stringWithFormat:@"%d",i]];
