...
-(BOOL)textFieldShouldEndEditing:(UITextField *)textField
{
    ...
    if (cell.tag==1) {
        ...
        if (cell.codeText.text.length > 0) {
            if ([self.allWorkingPremiseArray containsObject:cell.codeText.text])// && (cell.codeText.text.length > 0))
            {
    ...
    if (cell.tag==2) {
        ...
        if (cell.codeText.text.length > 0)
        {
            if ([self.departmentCodeArray containsObject:cell.codeText.text])// && (cell.codeText.text.length > 0))
            {
    ...
    if (cell.tag==3) {
        ...
        if (cell.codeText.text.length > 0) {
            if ([self.categoryCodeArray containsObject:cell.codeText.text])// && (cell.codeText.text.length > 0))
            {
