...
-(void)initAccordion{
    ...
    if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"expenseAction"]isEqualToString:@"create"]) {
...
- (void)setSelectedIndex:(NSInteger)aSelectedIndex {
    ...
    expenseReimbursementSettingsViewClass *ob = (expenseReimbursementSettingsViewClass *)self.superview;
    [ob disable];
...
- (void)touchDown:(id)sender {
    ...
    NSMutableArray *categoryArray = [[NSUserDefaults standardUserDefaults]objectForKey:@"ExpenseArray"];
    if (categoryArray.count > 0) {
        ...
        if (myappde.warningflag==0) {
            ...
            for (UIView *myviews in self.scrollView.subviews) {
                if ([myviews isKindOfClass:[expenseReimbursementSettingsViewClass class]]) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"ExpenseArrayReloade" object:Nil];
...
-(void)addaccordianview
{
    ...
    if (storevalue==flag-2) {
        ...
        NSArray *array=[[NSBundle mainBundle]loadNibNamed:@"expenseSettingsInnerView" owner:self options:nil];
...
-(void)deletetile:(UILongPressGestureRecognizer *)sender
{
    ...
    mytag=btn.tag;
    NSLog(@"%i",mytag);
    myappde.conditionID=[myappde.conditionIDArray objectAtIndex:mytag];
    if ((headers.count > 1) && (!([myappde.conditionID isEqualToString:@"0"]))) {
        ...
        closebutton.contentEdgeInsets = UIEdgeInsetsMake(1.5,1.5,1.5,1.5);
        closebutton.tag=100;
        deleteflag=1;
...
-(void)deletebuttonaction
{
    ...
    UIAlertAction* ok = [UIAlertAction actionWithTitle:@"Delete" style:UIAlertActionStyleDefault
                                               handler:^(UIAlertAction * action){
                                                   ...
                                                   [self.myconnection deleteExpenseCondition:[myappde.conditionIDArray objectAtIndex:mytag]];
