...
@implementation performanceEvaluationTile
...
-(void)addaccordianview
{
    ...
    if (selectedIndexValue==flag-2) {
        ...
        label.text=[NSString stringWithFormat:@"Evaluation Category %d",flag];
        ...
        NSArray *array=[[NSBundle mainBundle]loadNibNamed:@"performanceInnerView" owner:self options:nil];
    ...
    for (int i=0; i< headers.count; i++) {
        ...
        for (UILabel *myview in button.subviews) {
            if ([myview isKindOfClass:[UILabel class]]) {
                myview.text=[NSString stringWithFormat:@"Evaluation Category %d",i+1];
            }
